        ...
        for s in root.iter('SECTION') :
            for c in s.iter('CONTROL') :
                ref = {
                    ...
                    'CID' : c.find('ID').text,
                    'evaluation' : ET.tostring(c.findall("TECHNOLOGIES/TECHNOLOGY/EVALUATE")[0])
                }
                control_list_qualys.append(ref)
                print(ref)

control_list_qualys=unique(control_list_qualys)
print(f"Nombre de Controles du catalogues CIS Qualys : {len(control_list_qualys)}")

path = 'WS2019/CIS_Tenable/'
list_file = os.scandir(path)
CIS_Tenable= []
for item in list_file:
    if item.is_file():
...
print(f"Nombre de Controles du catalogues CIS Tenable : {len(CIS_Tenable)}")
...
path = 'WS2019/Custom_Qualys/'
tree = ET.parse(path+'ICDC_STD_SSI_Windows_Server_2019_2021_V2_1_20220602.xml')
root = tree.getroot()
qualys_custom = []
for s in root.iter('SECTION') :
    for c in s.iter('CONTROL') :
...
print(f'Nombre de controle Windows Server 2019 MS existant dans Tenable: {i} \ntaux de couverture = {round(i/len(final)*100)}%')
